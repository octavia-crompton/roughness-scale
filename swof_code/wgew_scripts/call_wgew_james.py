...
label = 175
meta = pd.read_csv(
    '/Users/octaviacrompton/Google_Drive_quatratavia/data/polyakov2017rainfall/rain/meta.csv')
...
out_name = "calibrate_{0}_J".format(key)
...
# Cd = meta.C/(9.8*8)
# Cd = list(np.round(np.linspace(Cd/10, Cd, 10)))

phi_veg = list(np.round(np.arange(0.0, 0.3, 0.05), 2))
...
sim_dict = {"phi_veg" : phi_veg, 
            "Ks_v" : Ks_v,
            "dthetacoef" : [0, 0.025, 0.05, 0.1]
            }
...
def call_SWOF(sdict):
    ...
    if case["phi_veg"] > 0:
        case["R_v"] = np.pi/4*(1-case["phi_veg"])*case["D"]/case["phi_veg"]    
        case["alpha_o"] = get_alpha_o(case["D"], case["phi_veg"])    
    else:
        case["R_v"] = 0
        case["alpha_o"] = 0
