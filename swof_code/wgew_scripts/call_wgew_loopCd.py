...
def call_SWOF(args ):
    ...
    out_name = 'calibrate_Cd_7/{0}'.format(key)
...
if __name__ == '__main__':
    ...
    for label in meta.label:
        ...
        out_name = key
        out_dir = os.path.join(project_dir , 'calibrate_Cd_7', out_name)
        folder =  os.path.join(project_dir , 'calibrate_Cd_7')
        ...
        Cd = np.round(np.linspace(row.Cd_avg*0.25**2, row.Cd_avg*1.5**2, 15), 3)
        Cd = list(Cd)
        ...
        sim_vars = [s for s in sim_dict.keys() if s != 'Cd'] 

        sim_list0 = [(dict(list(zip(sim_vars, prod)))) for prod in
           zip(*(sim_dict[var_name] for var_name in sim_vars))]

        sim_list= []
        for i, Cd in enumerate(sim_dict['Cd']):  
            sim_list = sim_list + [dict(item, **{'Cd' : Cd, 'So' : So}) for item in sim_list0]
