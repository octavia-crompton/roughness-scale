...
def call_SWOF(args ):
    ...
    out_name = "calibrate_n__7/{0}".format(key)
...
if __name__ == "__main__":
    ...
    for label in meta.label:
        ...
        out_name = key
        out_dir = os.path.join(project_dir , "calibrate_n__7", out_name)
        folder =  os.path.join(project_dir , "calibrate_n__7")
        ...
        n = np.round(np.linspace(row.n_reg*0.25, row.n_reg*1.5, 15), 3)
        n = list(n)
        ...
        sim_list= []
        for i, n in enumerate(sim_dict['n']):  
            sim_list = sim_list + [dict(item, **{'n':n, 'So' : So}) for item in sim_list0]
