...
def call_SWOF(args ):
    ...
    out_name = "calibrate_L_7/{0}".format(key)
...
if __name__ == "__main__":
    ...
    for label in meta.label:
        ...
        out_name = key
        out_dir = os.path.join(project_dir , "calibrate_L_7", out_name)
        folder =  os.path.join(project_dir , "calibrate_L_7")
        ...
        K = row.K_reg
        K = list(np.round(np.linspace(K*0.25, K*2, 15), 1))
        ...
        sim_vars = [s for s in sim_dict.keys() if s != 'K'] 
        ...
        sim_list= []
        for i, K in enumerate(sim_dict['K']):  
            sim_list = sim_list + [dict(item, **{'K':K, 'So' : So}) for item in sim_list0]
