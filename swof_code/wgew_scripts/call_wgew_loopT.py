...
def call_SWOF(args ):
    ...
    out_name = "calibrate_T__7/{0}".format(key)
    ...
    alpha_v = case['m1'] # *1e-6/8

    case['alpha_v'] = alpha_v
    case['alpha_b'] = alpha_v
    case['inf'] = 2
    case['dthetacoef'] = case['H']/100.
...
if __name__ == "__main__":
    ...
    for label in meta.label:
        ...
        out_name = key
        out_dir = os.path.join(project_dir , "calibrate_T__7", out_name)
        folder =  os.path.join(project_dir , "calibrate_T__7")
        ...
        # m1 = 8*9.8*row.m1_reg**2
        # m1 = np.round(np.linspace(m1*0.25, m1*2, 15), 4)
        # m1 = list(m1)
        m1 = np.linspace(row.m1_reg*0.25, row.m1_reg*2, 15) 
        m1 = np.round(8*9.8*m1**2, 4)
        m1 = list(m1)


        # this is the sorptive case
        ...
        sim_vars = [s for s in sim_dict.keys() if s != 'n'] 
        ...
        sim_list= []
        for i, m1 in enumerate(sim_dict['m1']):  
            sim_list = sim_list + [dict(item, **{'m1':m1, 'So' : So}) for item in sim_list0]
        sim_list = [(s, key) for s in sim_list]

        # sim_list = [(dict(list(zip(sim_vars, prod))), key) for prod in
