...
# key = 'TA-2-499' 
# key = 'ER4S-1-242'  ## Empire ranch
label = 499
meta = pd.read_csv(
    '/Users/octaviacrompton/Dropbox/data/polyakov2017rainfall/meta_new.csv')
...
out_name = "calibrate_n/{0}".format(key)
...
print(row.n_reg)
n = np.round(np.linspace(row.n_reg/2, row.n_reg, 10), 2)
n = list(n)

Ks_v = row['Ks']/10
Ks_v  = np.arange(Ks_v - 0.5, Ks_v + 0.25, 0.25)
Ks_v = list(np.round(Ks_v[Ks_v >0], 2))

try:
    default['So'] = row['Slope']
except:
    default['So'] = 0.1
