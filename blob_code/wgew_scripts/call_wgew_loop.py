...
def call_SWOF(args ):
    ...
    out_name = "calibrate_dw_8/{0}".format(key)
...
if __name__ == "__main__":
    ...
    for label in meta.label:
        ...
        out_name = key
        out_dir = os.path.join(project_dir , "calibrate_dw_8", out_name)
        folder =  os.path.join(project_dir , "calibrate_dw_8")
        ...
        dw = row.f_avg
        dw = dw*0.7
        dw = np.round(np.linspace(dw*0.25**2, dw*1.5**2, 16 ), 2)
        dw = list(dw) 
