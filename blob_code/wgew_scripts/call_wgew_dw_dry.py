...
label = 102
meta = pd.read_csv(
    '/Users/octaviacrompton/Dropbox/data/polyakov2017rainfall/meta_dry.csv')
...
out_name = "calibrateD_{0}_dw".format(key)
...
dw_ff = np.round(np.linspace(meta.dw/5, meta.dw*2, 10 ))
dw_ff = list(dw_ff)

# K = meta.K
# K = list(np.round(np.linspace(K/5, K*2, 10)))
...
sim_dict = {"alpha_v" : dw_ff,        
            "Ks_v" : Ks_v,
            "dthetacoef" : [0, 0.1, 0.2, 0.3]
            }
